...
namespace KMMobile.ZipLib.Zip.Compression 
{
		...
		int InsertString() 
		{
			...
			if (DeflaterConstants.DEBUGGING) 
			{
				if (hash != (((window[strstart] << (2*HASH_SHIFT)) ^ 
						...
						throw new ZipBaseException("hash inconsistent: " + hash + "/"
												+window[strstart] + ","
												+window[strstart + 1] + ","
					...
					if (DeflaterConstants.DEBUGGING) 
					{
						for (int i = 0 ; i < matchLen; i++) {
							if (window[strstart + i] != window[matchStart + i]) {
								throw new ZipBaseException("Match failure");
							}
						}
					}
#endif					
		...
		bool DeflateSlow(bool flush, bool finish)
		{
			...
			while (lookahead >= MIN_LOOKAHEAD || flush) {
				if (lookahead == 0) {
					...
					{
						throw new ZipBaseException("Not flushing, but no lookahead");
					}
#endif               
					huffman.FlushBlock(window, blockStart, strstart - blockStart,
						finish);
